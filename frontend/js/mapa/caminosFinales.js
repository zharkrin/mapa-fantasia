// frontend/js/mapa/caminosFinales.js
import { coloresTerreno } from "./coloresTerreno.js";

/**
 * Genera caminos finales entre regiones, curvos, evitando obstáculos y con interpolación suave
 * @param {Array} regiones - lista de regiones con {centro, tipo}
 * @param {number} ancho - ancho del mapa
 * @param {number} alto - alto del mapa
 * @param {number} gridSize - tamaño de cuadrícula para A*
 * @param {number} maxDist - distancia máxima para generar caminos
 * @returns {Array} caminos finales [{origen, destino, ruta, tipo}]
 */
export function generarCaminosFinales(regiones, ancho, alto, gridSize = 20, maxDist = 250) {
    const caminos = [];

    // Crear cuadrícula de costos
    ...
    for (let i = 0; i < regiones.length; i++) {
        for (let j = i + 1; j < regiones.length; j++) {
            ...
            if(dist <= maxDist){
                ...
                const rutaSuave = suavizarRuta(ruta);

                caminos.push({origen: r1, destino: r2, tipo, ruta: rutaSuave});
            }
        }
    }
...
/**
 * Dibuja los caminos finales en canvas
 * @param {CanvasRenderingContext2D} ctx
 * @param {Array} caminos
 */
export function dibujarCaminosFinales(ctx, caminos){
    caminos.forEach(c => {
        switch(c.tipo){
...
function aStarRuta(start, end, grid, gridSize){
    ...
    while(open.length>0){
        ...
        if(current.x===endNode.x && current.y===endNode.y){
            const path=[];
            let node=current;
            while(node){ path.push({x:node.x*gridSize+gridSize/2, y:node.y*gridSize+gridSize/2}); node=node.parent; }
            return path.reverse();
        }
